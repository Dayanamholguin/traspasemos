                ...
                <div class="row">
                    ...
                    <div class="col-md-12 container">
                        <p class="float-start"><strong>Nombre:</strong> <a class="float-right text-dark" style="text-decoration: none;" id="nombre"></a></p>
                            ...
                            <p class="float-start"><strong>Tipo de usuario:</strong> <a class="float-right text-dark" style="text-decoration: none;" id="tipoUsuario"></a></p>
                        </div>
                    </div>
                    <div class="col-md-12 text-center">
                        <hr>
...
<script>
    ...
    function mostrarVentana(id) {
        ...
        // moment.locale('es');
        $.ajax({
            url: `/usuario/verUsuario/${id}`,
            type: "GET",
            success: function(res) {
                $('#nombre').html(res.usuario.nombre);
                $('#apellido').html(res.usuario.apellido);
                $('#email').html(res.usuario.email);
                $('#tipoDocumento').html(res.tipoDocumento);
                $('#numeroDocumento').html(res.usuario.numeroDocumento);
                // $('#creacion').html(res.usuario.created_at);
                $('#tipoUsuario').html(res.tipoUsuario);
                // $('#fecha').html(ucwords(new Date(res.abono.updated_at).toLocaleDateString("es-ES", options)));
                // $('#creacion').html(ucwords(new Date(res.usuario.created_at).toLocaleDateString("es-ES", options)));
                // $('#creacion').html(moment(res.usuario.created_at,'DD/MM/YYYY HH:mm:ss a').format('DD/MM/YYYY HH:mm:ss a'));       
                // $('#creacion').html(moment(res.usuario.created_at).format('MMMM Do YYYY, h:mm:ss a'));       
                $('#creacion').html("Creado en "+moment(res.usuario.created_at).format('MM DD YYYY'));       
                // $('#creacion').html(moment.parseZone("2013 01 01 -13:00", 'YYYY MM DD ZZ', 'fr', true).utcOffset());
                
                // $('#tipoUsuario').html($.datepicker.formatDate('dd M yy', new Date(res.usuario.created_at)));
            },
        });
    }
</script>
